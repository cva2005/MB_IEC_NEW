...
static bool LinkTxFrame = false;
static uint8_t* pBuff;
static int txLen;

static void LinkTxSet(uint8_t *buff, uint16_t len)
{
	LinkTxFrame = true;
	pBuff = buff;
	txLen = len;
...
uint8_t *LinkTxNeed(uint16_t *len)
{
	if (LinkTxFrame)
	{
		LinkTxFrame = false;
...
SendSingleCharCharacter(LinkLayer self)
{
	...
	buff[0] = 0xe5;
	LinkTxSet(buff, 1); // Delayed Send Message
}
...
SendFixedFrame(LinkLayer self, uint8_t fc, int address, bool prm, bool dir, bool acd /*FCB*/, bool dfc /*FCV*/)
{
	...
	LinkTxSet(buffer, bufPos); // Delayed Send Message
}
