...
#error Illegal configuration: Define either CONFIG_CS104_SUPPORT_SERVER_MODE_SINGLE_REDUNDANCY_GROUP or\
    CONFIG_CS104_SUPPORT_SERVER_MODE_SINGLE_REDUNDANCY_GROUP or\
    CONFIG_CS104_SUPPORT_SERVER_MODE_MULTIPLE_REDUNDANCY_GROUPS
#endif
...
struct sCS104_APCIParameters defaultConnectionParameters;
#if 0
...
{
	CS104_Slave slave = createSlave(maxLowPrioQueueSize, maxHighPrioQueueSize);
    slave->masterConnections[0]->slave = slave;
	return slave;
}
...
#else
    return Socket_write(NULL, buf, size);
#endif
}
...
handleMessage(MasterConnection self, uint8_t* buffer, int msgSize)
{
    if (msgSize >= 3) {
        ...
        /* Check for TESTFR_CON message */
            ...
            if (checkSequenceNumber(self, seqNo) == true)
	            resetT3Timeout(self, currentTime);
	        return false;
        }
...
MasterConnection_create(CS104_Slave slave)
{
    ...
    if (self != NULL) {
        ...
        self->iMasterConnection.getPeerAddress = /*_IMasterConnection_getPeerAddress*/NULL;

#if (CONFIG_USE_SEMAPHORES == 1)
        self->sentASDUsLock = Semaphore_create(1);
#endif
        self->handleSet = /*Handleset_new()*/NULL;
...
{
#if 1/*((CONFIG_USE_THREADS == 1) && (CONFIG_USE_SEMAPHORES == 1))*/
    if (self->isRunning == false) {
...
int
CS104_Slave_getNumberOfHighQueueEntries(CS104_Slave self)
{
    return self->masterConnections[0]->highPrioQueue->entryCounter;
}
...
#else
	    if (self->serverMode == CS104_MODE_SINGLE_REDUNDANCY_GROUP) {
		    MessageQueue lowPrioQueue = self->asduQueue;
		    HighPriorityASDUQueue highPrioQueue = self->connectionAsduQueue;
		    MasterConnection connection = getFreeConnection(self);
		    if (connection) {
			    MasterConnection_init(connection, NULL, lowPrioQueue, highPrioQueue);
			    connection->isRunning = true;
                connection->socket = TcpSocket_create();
            }
	    }
    }
#endif
...
void
CS104_Slave_executePeriodicTasks(CS104_Slave self)
{
    if (iec_socket_is_connected())
        MasterConnection_executePeriodicTasks(self->masterConnections[0]);
	else
	    CS104_Slave_removeConnection(self, self->masterConnections[0]);
}
...
void
CS104_ParseMessage(CS104_Slave slave, uint8_t *buff, int len)
{
	if (slave->rawMessageHandler)
		slave->rawMessageHandler(slave->rawMessageHandlerParameter,
			buff,
			len,
			false);

	MasterConnection self = slave->masterConnections[0];

    (void)handleMessage(self, buff, len);

    if (self->unconfirmedReceivedIMessages >= slave->conParameters.w) {
...
void
CS104_MasterConnectionReset(CS104_Slave slave)
{
    MasterConnection self = slave->masterConnections[0];
    MasterConnection_init(self, NULL, self->lowPrioQueue, self->highPrioQueue);
    self->isRunning = true;
}