 ...
 * iec_util_set_bits(ucBits, 4, 1, 1);
 *
 * // Set bit 7 to 1 and bit 8 to 0.
 * iec_util_set_bits(ucBits, 7, 2, 0x01);
 *
 * // Set bits 8 - 11 to 0x05 and bits 12 - 15 to 0x0A;
 * iec_util_set_bits(ucBits, 8, 8, 0x5A);
 * \endcode
 */
void iec_util_set_bits(uint8_t *byte_buf, uint16_t bit_offset, uint8_t but_num, uint8_t values);
 ...
 * ucResult = iec_util_get_bits(ucBits, 3, 8);
 * \endcode
 */
uint8_t iec_util_get_bits(uint8_t *byte_buf, uint16_t bit_offset, uint8_t but_num);
 ...
 *
 * returns the modbus error code = IEC_ENOERR, if set correctly, IEC_ENOREG, otherwise
 * \endcode
 */
